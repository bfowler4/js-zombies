...
function Player(name, health, strength, speed) {
  ...
  this.isItemInPack = function(item) {
    let currentPack = this.getPack();
    if (currentPack.indexOf(item) !== -1) {
      return currentPack.indexOf(item);
    } else {
      return false;
    }
  }
  ...
  Player.prototype.checkPack = function() {
    ...
    currentPack.forEach(function(curr) {
      console.log(`  Item: ${curr.name}`);
    });
  }
  ...
  Player.prototype.discardItem = function(item) {
    let itemIndex = this.isItemInPack(item);
    if (itemIndex === false) {
      console.log(`Item: ${item.name} was not found in pack.`);
      return false;
    } else {
      console.log(`Player: ${this.name}\n${item.name} was discarded.`);
  ...
  Player.prototype.equip = function(itemToEquip) {
    ...
    if (itemToEquip instanceof(Weapon) && itemIndex !== false) {
      if (this.equipped) {
        this.getPack()[itemIndex] = this.equipped;
      } else {
        this.getPack().splice(itemIndex, 1);
      }
  ...
  Player.prototype.eat = function(itemToEat) {
    ...
    if (itemToEat instanceof(Food) && itemIndex !== false) {
      this.getPack().splice(itemIndex, 1);
      this.health = this.health + itemToEat.energy > this.getMaxHealth() ? this.getMaxHealth() : this.health + itemToEat.energy;
    }
  }
  ...
  Player.prototype.useItem = function(item) {
    if (this.isItemInPack(item) !== false) {
  ...
  Player.prototype.equippedWith = function() {
    if (this.equipped) {
      console.log(`Player: ${this.name}, Equipped: ${this.equipped.name}`);
      return this.equipped.name;
    } else {
      console.log(`Player: ${this.name} has nothing equipped.`);
      return false;
    }
  }
}
